...
A step-by-step reference guide for setting up a **TypeScript backend** with **Express**, **Prisma ORM**, and **npm** as the package manager.
...
| 2   | Run a Migration          | `npx prisma migrate dev`    |
| 3   | Generate Prisma Client   | `npx prisma generate`       |
| 4   | Open Prisma Studio (GUI) | `npx prisma studio`         |
| 5   | Deploy (Production Only) | `npx prisma migrate deploy` |
...
> [!NOTE]
> Unlike Bun, Node.js does not automatically load `.env` files. The `dotenv` package is required, and `import 'dotenv/config'` (or `dotenv.config()`) must be called at the entry point of your application.
...
> [!IMPORTANT]
> When using npm, always use `npx` to run Prisma CLI commands. `npx` executes the locally installed binary from `node_modules/.bin/`.
...
1. Create a Prisma Postgres database via the Prisma Console
...
> [!TIP]
> Re-run `npx prisma generate` any time you modify your schema file(s) to keep your Prisma Client in sync.
...
> [!NOTE]
> If you need to query your database via HTTP from an edge runtime (e.g., Cloudflare Workers, Vercel Edge Functions), use the Prisma Postgres serverless driver instead of the `pg` adapter.
...
| ----------------- | ---------------------------- | ------------------------------------------------------------------------ |
| `npm run dev`     | `tsx watch src/server.ts`   | Starts the dev server with hot reload — restarts on file changes        |
| `npm run build`   | `tsc`                        | Compiles TypeScript source files into JavaScript inside `dist/`         |
| `npm run start`   | `node dist/server.js`       | Runs the compiled production build from `dist/`                         |

> [!NOTE]
> `tsx watch` is used for development to provide fast TypeScript execution with automatic restarts on file changes, without requiring a separate compilation step.

> [!WARNING]
> Always run `npm run build` before `npm run start`. The `start` script runs the compiled `.js` output from `dist/` — if `dist/` is missing or outdated, it will fail.
